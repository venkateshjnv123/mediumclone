...
function Myposts(){
    ...
    posts ?
     posts.map((blog)=>(
        <div onClick={()=>handleshowpost(blog)} className="md:w-3/5 border-b p-4 mb-4 bg-slate-200 rounded-lg shadow-lg cursor-pointer">
        {/* Author Name */}
 ...
 <div className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full cursor-pointer">
    ...
    <div className="flex"> 
           ...
           <div>
              ...
              <div>
              <GrView className="text-slate-200" size={28} />
              </div>
              <div className="pl-[5px]">
                <span>{blog.views}</span>
              </div>
    ...
    <button className='rounded-full bg-slate-400 w-24 h-8' onClick={()=>handledelete(blog.id)}>Delete</button>
    </div>
